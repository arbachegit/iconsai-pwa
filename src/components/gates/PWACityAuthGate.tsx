...
import { Loader2, RefreshCw, MessageSquare, Phone, ArrowLeft, AlertTriangle, CheckCircle2 } from "lucide-react";
import { usePWACityAuth, CodeSentChannel } from "@/hooks/usePWACityAuth";
...
function CodeSentFeedback({
  ...
  if (channel === 'sms') {
    return (
      <div className="flex items-center gap-2 bg-cyan-500/10 text-cyan-500 p-3 rounded-lg mb-4">
...
function LoginScreen({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 flex flex-col items-center justify-center p-4">
      <div className="max-w-md w-full bg-slate-900/50 backdrop-blur-md rounded-2xl p-8 shadow-xl border border-cyan-500/20">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white mb-2">PWA City</h1>
        ...
        <div className="mt-6 text-center">
          <p className="text-xs text-slate-500">
            Não tem acesso? O PWA City funciona apenas por convite.
          </p>
        </div>
      </div>
    </div>
...
function SendingCodeScreen({ phone }: { phone: string }) {
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 flex flex-col items-center justify-center p-4">
      <div className="max-w-md w-full bg-slate-900/50 backdrop-blur-md rounded-2xl p-8 shadow-xl border border-cyan-500/20 text-center">
        <div className="w-20 h-20 bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 rounded-full flex items-center justify-center mx-auto mb-6 relative">
          <MessageSquare className="h-10 w-10 text-cyan-400 animate-pulse" />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 flex flex-col items-center justify-center p-4">
      <div className="max-w-md w-full bg-slate-900/50 backdrop-blur-md rounded-2xl p-8 shadow-xl border border-cyan-500/20">
        <button
          onClick={onBack}
          className="mb-4 flex items-center gap-2 text-slate-400 hover:text-cyan-400 transition-colors"
        >
...
export function PWACityAuthGate({ children }: PWACityAuthGateProps) {
  ...
  useEffect(() => {
    if (isDemoMode && demoType) {
      console.log("[PWACityAuthGate] Demo mode detectado:", demoType);
  ...
  if (isDemoMode) {
    console.log("[PWACityAuthGate] Bypass de autenticação (demo mode)");
  ...
  console.log("[PWACityAuthGate] Status:", status);

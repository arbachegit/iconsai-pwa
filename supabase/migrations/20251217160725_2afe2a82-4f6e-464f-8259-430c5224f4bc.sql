-- Update the PMC conversion function to use pac_valores_estimados for PAC data
...
BEGIN
  ...
  -- Insert converted PMC values using PAC estimated data (now includes 2024-2025)
  ...
  SELECT 
    ...
    EXTRACT(MONTH FROM irv.reference_date)::INTEGER as reference_month,
    ...
    -- Formula: (PMC_index / 100) * (PAC_annual_revenue / 12)
  ...
  SELECT 
    ...
    MIN(pv.reference_year)::text || '-' || MAX(pv.reference_year)::text as years_covered
